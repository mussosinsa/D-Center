 ...
 * @version 2.0
 */
public class WebSystemMetricsJob extends QuartzJobBean {

    /**
     * SLF4J Logging
     */
    private Logger logger = LoggerFactory.getLogger(WebSystemMetricsJob.class);

    @Override
    protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        logger.info("Flamingo Web의 시스템 메트릭스 정보를 수집합니다.");
        ...
        for (SystemConfig config : values) {
            ...
            String url = MessageFormatter.arrayFormat("http://{}:{}/monitoring/flamingo/web.json", new Object[]{address, port}).getMessage().toString();
            RestTemplate restTemplate = new RestTemplate();
            ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
            try {
                Map root = new ObjectMapper().readValue(response.getBody().getBytes(), Map.class);
                Map map = (Map) root.get("map");
                service.collect(config.getId(), config.getName(), "web", map);
            } catch (Exception e) {
                throw new RuntimeException("Flamingo Web에서 시스템 메트릭스 정보를 확인할 수 없습니다.", e);
            }
        }
    }
}
