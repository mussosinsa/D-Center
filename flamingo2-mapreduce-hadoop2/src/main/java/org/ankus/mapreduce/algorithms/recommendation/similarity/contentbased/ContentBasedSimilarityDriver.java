 ...
 * @author Suhyun Jeon
 * @version 0.1
 * @desc Here's an version of the similarity coefficient and distance calculation.
...
public class ContentBasedSimilarityDriver extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        if (args.length < 1) {
            Usage.printUsage(Constants.ALGORITHM_CONTENT_BASED_SIMILARITY);
            return -1;
        }
        ...
        job1 = HadoopUtil.prepareJob(job1, new Path(input), prepareOutputPath, ContentBasedSimilarityDriver.class,
                ContentBasedSimilarityMapper.class, Text.class, TextTwoWritableComparable.class,
                ContentBasedSimilarityReducer.class, TextTwoWritableComparable.class, TextFourWritableComparable.class);

        job1.getConfiguration().set(Constants.DELIMITER, delimiter);
        job1.getConfiguration().set(Constants.SUB_DELIMITER, subDelimiter);
        job1.getConfiguration().set(Constants.KEY_INDEX, keyIndex);
        job1.getConfiguration().set(Constants.TARGET_INDEX, indexList);
        job1.getConfiguration().set(Constants.ALGORITHM_OPTION, algorithmOption);
        ...
        job2 = HadoopUtil.prepareJob(job2, prepareOutputPath, new Path(output), ContentBasedSimilarityDriver.class,
                CalculationContentBasedSimilarityMapper.class, TextTwoWritableComparable.class, TextFourWritableComparable.class,
                CalculationContentBasedSimilarityReducer.class, TextTwoWritableComparable.class, DoubleWritable.class);
        ...
        try {
            for (int i = 0; i < args.length; ++i) {
                if (ArgumentsConstants.INPUT_PATH.equals(args[i])) {
                    ...
                    keyIndex = args[++i];
                } else if (ArgumentsConstants.TARGET_INDEX.equals(args[i])) {
                    indexList = args[++i];
                    ...
                    delimiter = args[++i];
                } else if (ArgumentsConstants.SUB_DELIMITER.equals(args[i])) {
                    subDelimiter = args[++i];
                }
            }
        } catch (Exception e) {
