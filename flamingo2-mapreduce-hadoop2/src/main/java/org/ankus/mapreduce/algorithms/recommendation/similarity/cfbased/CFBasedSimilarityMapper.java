...
public class CFBasedSimilarityMapper extends Mapper<LongWritable, Text, Text, TextDoublePairWritableComparable> {
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (basedType.equals(Constants.USER_BASED)) {
            keyID.append(columns[0]);
            itemID.append(columns[1]);
        } else if (basedType.equals(Constants.ITEM_BASED)) {
            keyID.append(columns[1]);
            itemID.append(columns[0]);
        }

        TextDoublePairWritableComparable textDoublePairWritableComparable = new TextDoublePairWritableComparable(keyID.toString(), rating);
        context.write(new Text(itemID.toString()), textDoublePairWritableComparable);
    }
