...
import org.ankus.mapreduce.algorithms.recommendation.recommender.candidate.CandidateMapper;
import org.ankus.mapreduce.algorithms.recommendation.recommender.candidate.CandidateReducer;
...
import org.ankus.mapreduce.algorithms.recommendation.recommender.neighborhood.itembased.ItembasedMovielensMapper;
import org.ankus.mapreduce.algorithms.recommendation.recommender.neighborhood.itembased.ItembasedNeighborhoodMapper;
import org.ankus.mapreduce.algorithms.recommendation.recommender.neighborhood.itembased.ItembasedNeighborhoodReducer;
...
public class RecommendationDriver extends Configured implements Tool {
    ...
    // SLF4J Logging
    private Logger logger = LoggerFactory.getLogger(RecommendationDriver.class);

    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new RecommendationDriver(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job1 = HadoopUtil.prepareJob(job1, new Path(input), itemListOutputPath, RecommendationDriver.class,
        ...
        job2 = HadoopUtil.prepareJob(job2, new Path(similarDataInput), switchSimilarityOutput1, RecommendationDriver.class,
        ...
        job3 = HadoopUtil.prepareJob(job3, new Path(similarDataInput), switchSimilarityOutput2, RecommendationDriver.class,
        ...
        job4 = HadoopUtil.prepareJob(job4, switchSimilarityOutput1, switchSimilarityOutput2, aggregateSwitchSimOutput, RecommendationDriver.class,
        ...
        if (basedType.equals(Constants.USER_BASED)) {
            job5 = HadoopUtil.prepareJob(job5, new Path(input), aggregateSwitchSimOutput, neighborAllDataOutput, RecommendationDriver.class,
            ...
            job5 = HadoopUtil.prepareJob(job5, new Path(input), aggregateSwitchSimOutput, neighborAllDataOutput, RecommendationDriver.class,
                    ItembasedMovielensMapper.class, ItembasedNeighborhoodMapper.class, Text.class, Text.class,
                    ItembasedNeighborhoodReducer.class, Text.class, Text.class);
        }
         ...
         */
        logger.info("==========================================================================================");
        logger.info("   Step 6 of the 7 steps : Arrange candidate items for n users. ");
        logger.info("       Input directory [" + input + "]");
        ...
        Job job6 = new Job(this.getConf());
        job6 = HadoopUtil.prepareJob(job6, new Path(input), candidateItemListOutput, RecommendationDriver.class,
                CandidateMapper.class, Text.class, TextTwoWritableComparable.class,
                CandidateReducer.class, Text.class, Text.class);
        ...
        job7 = HadoopUtil.prepareJob(job7, candidateItemListOutput, neighborAllDataOutput, new Path(output), RecommendationDriver.class,
        ...
        if (removeMode) {
            ...
            if (delete) {
                logger.info("Deleted midterm process output files.");
            }
        }
        return 0;
    }
