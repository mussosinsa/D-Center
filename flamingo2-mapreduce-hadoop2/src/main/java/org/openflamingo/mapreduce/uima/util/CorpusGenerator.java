...
/**
 * Generates a SequenceFile containing UIMADocuments given a local
 * directory. The UIMADocument gets its byte content and URL. The detection
...
public class CorpusGenerator extends Configured implements Tool {
    ...
    public long generate(boolean recurse) throws IOException {
        ...
        Text key = new Text();
        UIMADocument value = new UIMADocument();
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(UIMAConfiguration.create(), new CorpusGenerator(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        options.addOption("o", "output", true, "output UIMA corpus");
        ...
        options.addOption("md", "metadata", true, "add document metadata separated by semicolon e.g. -md source=internet;label=public");
    ...
    static class PerformanceFileFilter implements PathFilter {
        ...
        private Text key;
        private UIMADocument value;
        private Configuration conf;
        private Reporter reporter;

        public PerformanceFileFilter(SequenceFile.Writer writer, Text key,
                                     UIMADocument value, Configuration conf, Reporter reporter) {
            this.writer = writer;
            this.key = key;
            this.value = value;
            this.conf = conf;
        ...
        public boolean accept(Path file) {
            try {
                ...
                if (defaultIgnores.accept(file)
                    ...
                    if (unpack) {
                        if (uri.endsWith(".cpio") || uri.endsWith(".jar")
                                ...
                                || uri.endsWith("tar") || uri.endsWith(".zip")
                                || uri.endsWith("tar.gz")
                                || uri.endsWith(".tgz")
                                || uri.endsWith(".tbz2")
                                || uri.endsWith(".tbz")
                                || uri.endsWith("tar.bzip2")) {
                ...
                // since all the work is done in the accept method here.
