...
guessData <- function (filepath){
  options <- fromJSON(args[10]);
  dataDelimiter <- options$dataDelimiter;
  if (args[9] == 'true') {
    readData <- read.delim(filepath, header=T, sep=dataDelimiter, dec=".");
    return(list(data=readData,header=T, sep=dataDelimiter, dec=".", type="delim"));
  } else {
    readData <- read.delim(filepath, header=F, sep=dataDelimiter, dec=".");
    return(list(data=readData,header=F, sep=dataDelimiter, dec=".", type="delim"));
  }
}
...
makeRdata <- function(){
  ...
  filename <- paste(args[6], args[7],sep="/");
