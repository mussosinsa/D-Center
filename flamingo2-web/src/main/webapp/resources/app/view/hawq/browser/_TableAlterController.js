 ...
 */
/**
 * ViewController for Flamingo2.view.hawq.browser._TableAlter
 *
 * @author Ha Neul, Kim
 * @since 2.0
 * @see Flamingo2.view.hawq.browser._TableAlter
 */
Ext.define('Flamingo2.view.hawq.browser._TableAlterController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller._hawqTableAlterController',
    ...
    hawqConstraintStoreBeforeload: function (store, operation, eOpts) {
        var refs = this.getReferences();
        store.getProxy().extraParams.clusterName = ENGINE.id;
        store.getProxy().extraParams.databaseName = refs.databaseNameCombobox.getValue();
        store.getProxy().extraParams.schemaName = refs.schemaNameCombobox.getValue();
        ...
        invokeGet(
            ...
            function (response) {
                ...
                if (result.success) {
                    ...
                    // Setting With
                    var options = original.reloptions.split(','),
                        optionsMap = {};
                    for (var i = 0, length = options.length; i < length; i++) {
                        var option = options[i].split('=');
                        optionsMap[option[0]] = option[1];
                    }
                    refs.frmWith.options = optionsMap;

                    // Setting Partition
                    refs.hawqPartitionForm.getForm().setValues(original.partition);

                    // Setting Constraint
                    vm.getStore('hawqConstraint').reload();

                    // With Tab 이 활성화되지 않으면 data set 되지 않아서 임의로 활성화
                    refs.hawqTableAlterTabpanel.setActiveTab(1);
    ...
    hawqTableAlterWithFormAfterrender: function (formPanel) {
        var options = formPanel.options;
        if (options) {
            formPanel.getForm().setValues(options);
        }
    },

    hawqConstraintsGridAddButtonHandler: function (button) {
        var me = this,
            refs = me.getReferences(),
            grid = refs.hawqConstraintsGrid,
            cellediting = grid.getPlugin('alterTableConstraintsGridCellediting'),
            count = grid.getStore().getCount();
    ...
    hawqConstraintGridDeleteButtonHandler: function (button) {
        var me = this,
            refs = me.getReferences(),
            grid = refs.hawqConstraintsGrid,
            store = grid.getStore(),
        ...
        if (selection.length) {
            ...
            Ext.MessageBox.show({
                title: message.msg('hawq.title.warning'),
                message: message.msg('hawq.msg.warning.dropconstraint'),
                buttons: Ext.MessageBox.OK,
                icon: Ext.MessageBox.WARNING
            });
        }
    ...
    hawqConstraintsGridSaveButtonHandler: function (button) {
        var me = this,
            refs = me.getReferences(),
            grid = refs.hawqConstraintsGrid,
            store = grid.getStore();
        ...
        if (store.isValid()) {
            Ext.MessageBox.show({
                title: message.msg('hawq.title.alter'),
                message: message.msg('hawq.msg.question.alterconstraint'),
                buttons: Ext.MessageBox.YESNO,
                icon: Ext.MessageBox.QUESTION,
                fn: function (buttonId) {
                    if (buttonId === 'yes') {
                        ...
                        invokePostByMap(
                            CONSTANTS.HAWQ.BROWSER.ALTER_CONSTRAINT,
                            {
                                clusterName: ENGINE.id,
                                databaseName: refs.databaseNameCombobox.getValue(),
                                schemaName: refs.schemaNameCombobox.getValue(),
                            ...
                            function (response) {
                                ...
                                if (result.success) {
                                    Ext.MessageBox.show({
                                        title: message.msg('hawq.title.success'),
                                        message: message.msg('hawq.msg.success.alterconstraint'),
                                        buttons: Ext.MessageBox.OK,
                                        icon: Ext.MessageBox.INFO,
                                        fn: function () {
                                            grid.getStore().reload();
                                    ...
                                    Ext.MessageBox.show({
                                        title: message.msg('hawq.title.fail'),
                                        message: message.msg('hawq.msg.fail.alterconstraint') + '<br/>' + format(message.msg('hawq.msg.cause'), result.error.message),
                                        buttons: Ext.MessageBox.OK,
                                        icon: Ext.MessageBox.WARNING
                                    });
                                }
            ...
            Ext.MessageBox.show({
                ...
                icon: Ext.MessageBox.WARNING,
                fn: function () {
                    refs.hawqTableAlterTabpanel.setActiveTab(3);
                }
            });
        }
    },

    hawqConstraintsGridRefreshButtonHandler: function () {
        this.getReferences().hawqConstraintsGrid.getStore().reload();
    }
});