...
import org.opencloudengine.flamingo2.util.FileSystemScheme;
...
public class FileSystemUtils {
    ...
    public static String correctPath(String parent, String prefix, String filename) {
        if (isEmpty(parent) || isEmpty(prefix) || isEmpty(filename)) {
            return null;
        }
    ...
    public static void checkScheme(String path, FileSystemScheme scheme) {
        ...
        if (!path.startsWith(scheme.getScheme())) {
            throw new ServiceException(ExceptionUtils.getMessage("Invalid file system '{}' => Path '{}'", scheme.getScheme(), path));
        }
    }
     ...
     *
     * @param path 경로
     * @throws ServiceException 디렉토리를 생성할 수 없거나, 존재하지만 디렉토리가 아니라 파일인 경우
     */
    public static void testCreateDir(Path path) {
        try {
            ...
            if (fs.exists(path) && !fs.getFileStatus(path).isDirectory()) {
                throw new ServiceException(ExceptionUtils.getMessage("'{}' is not directory.", path));
            }

            if (!fs.exists(path)) {
                if (!fs.mkdirs(path)) {
                    throw new ServiceException(ExceptionUtils.getMessage("Cannot create '{}'", path));
                }
            }
        } catch (Exception ex) {
    ...
    public static FileSystem getFileSystem(Path path) {
        try {
            ...
            throw new ServiceException(ExceptionUtils.getMessage("Cannot get file system of '{}'", path), ex);
        }
    }
    ...
    public static void validateSameFileSystem(String path1, String path2) {
        ...
        try {
            ...
            throw new ServiceException(ExceptionUtils.getMessage("Cannot access '{}' or '{}'.", p1, p2), ex);
        }

        if (!compareFs(fs1, fs2)) {
            throw new ServiceException(ExceptionUtils.getMessage(
                    "File system is not same : {}, {}", p1, p2
            ));
        }

        if (p1.equals(p2)) {
            throw new ServiceException(ExceptionUtils.getMessage("Same path : {}, {}", p1, p2));
        }
    }
     ...
     * @return 파일의 내용
     * @throws ServiceException 파일을 로딩할 수 없는 경우
     */
    public static String load(String sourceFilePath) {
        ...
        try {
            ...
            throw new ServiceException(ExceptionUtils.getMessage("Cannot load '{}'.", sourceFilePath), ex);
        }
    }
     ...
     * @throws ServiceException 파일을 저장할 수 없는 경우
     */
