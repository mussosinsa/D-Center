 ...
 * @since 2.0
 */
public class SqoopExportTask extends InterceptorAbstractTask {

    /**
     * SLF4J Logging
     */
    private Logger logger = LoggerFactory.getLogger(SqoopExportTask.class);
    ...
    private String buildCommand(String working) {
        ...
        try {
            ...
            if (variable.get("output") != null && !StringUtils.isEmpty(variable.get("output").toString())) {
                command.add("--export-dir");
                command.add(variable.getProperty("output"));
            }

            if (variable.get("fieldDelimiter") != null && !StringUtils.isEmpty(variable.get("fieldDelimiter").toString())) {
                command.add("--fields-terminated-by");
                command.add(variable.getProperty("fieldDelimiter"));
            }

            if (variable.get("lineDelimiter") != null && !StringUtils.isEmpty(variable.get("lineDelimiter").toString())) {
                command.add("--lines-terminated-by");
                command.add(variable.getProperty("lineDelimiter"));
            }
        } catch (Exception e) {
            throw new ServiceException("You can not create a Sqoop export command.", e);
        }
