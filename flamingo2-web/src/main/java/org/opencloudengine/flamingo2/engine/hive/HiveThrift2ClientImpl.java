 ...
 */
public class HiveThrift2ClientImpl implements HiveThrift2Client {
    ...
    protected HiveThrift2ClientImpl(String uri) {
        try {
            this.jdbcUri = uri;
    ...
    public String getLog() throws TException {
        ...
        if (currentOperation == null)
            throw new ServiceException("Least one operation is executed.");
        ...
        // 쿼리의 실행 결과를 가져온다.
        TFetchResultsReq resultsReq = new TFetchResultsReq(currentOperation, TFetchOrientation.FETCH_NEXT, Integer.MAX_VALUE);
        resultsReq.setFetchType((short) 1);
        ...
        List<String> values = ((TStringColumn) cols[0].getFieldValue()).getValues();
        for (String v : values) {
    ...
    public Map[] getResults(TFetchOrientation orientation, int maxRows, FetchType fetchType) throws TException {
        ...
        resultsReq.setFetchType((short) 0);
    ...
    public static class Factory {
        public static HiveThrift2ClientImpl getClient(String uri) {
            return new HiveThrift2ClientImpl(uri);
        }
    }
}
